...
int connect_irc(int port, char *ipnum) {
	...
	inet_pton(AF_INET, ipnum, &address.sin_addr);
        ...
        if (connect(create_socket,(struct sockaddr *)&address,sizeof(address)) == 0)
        	...
        	fprintf(stderr,"The connection wasn't accepted by %s...\n",inet_ntoa(address.sin_addr));
        	return -2;
        }
...
int netsend(int socketnum, char *msg) {
  ...
  if(len<=1) {
  	fprintf(stderr,"string zu kurz. mindestlänge 2byte (CR-LF)!\n");
  	return -2;
  }
  
  else if (len>LINELENGTH) {
  	fprintf(stderr,"string zu lang. maximallänge %dbyte (inkl. CR-LF)! \n",LINELENGTH);
  	msg[LINELENGTH+1] ='\0';
  	msg[LINELENGTH]   ='\n';
  	msg[LINELENGTH-1] ='\r';
  	len=strlen(msg);
...
int resolve_ircd(char*host, int port) {
	struct hostent *he;      
        ...
        //he=NULL;
        if(he == NULL ) printf("Host %s got no IPv6 address\n",host);
        
       	else {                                                       
        	inet_ntop(AF_INET6, (void *)he->h_addr_list[0], ipv6, INET6_ADDRSTRLEN);
        	printf("Host %s got IPv6 address %s\nConnecting (IPv6)...\n",host,ipv6);
        	return(connect_irc6(port,ipv6));
        }                                   
        // Resolve IPv4
        he = gethostbyname(host);
        if(he == NULL) printf("Host %s got no IPv4 address.\n",host);
        else {                                                       
        	memcpy(&addr, he->h_addr_list[0], sizeof(struct in_addr));
                printf("Host %s got IPv4 address %s\nConnecting (IPv4)...",host,inet_ntoa(addr));
