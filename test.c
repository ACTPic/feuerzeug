	...
	while( (option = getopt(argc, argv , "-fh")) != EOF) { 
	  ...
	  switch(option) {
	 	...
	 	case 'f' : _FILE = TRUE; break;
	        ...
	        case '?' : fprintf(stderr,"\nfalscher schalter, %s -h ist dein freund",argv[0]);
	        		exit (1);
	        		break;
	        		
	        case  1  : if (_FILE == TRUE) {
	        			printf("versuche file. argc=%d i=%d argv=\n%s\n",argc,i,argv[i]);
