...
int main (int argc,char*argv[]) {
	...
	switch(_pid=fork()) {
		case -1	: fprintf(stderr,"fehler in fork()\n");
			  exit(-1);
			  break;
		case 0	: printf("kindprozess aktiv, aktiviere netrecv()\n");
			  while (1) {
			  	printf("%s",netrecv(_socket));
			  }
			  exit (0);
			  break;
		default	: printf("elternprozess aktiv. kind hat pid %d\n",_pid);
			  break;
	}
