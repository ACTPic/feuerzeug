...
int debug(struct vector *v) {
  ...
  while(elem!=NULL) {
    ...
    switch(elem->type) {
      case BF_TYPE_INT:
        printf("%i: %s - %i\n",elem->type,elem->name,node_toInt(node_copy(elem)));
        break;
      case BF_TYPE_FLOAT:
        printf("%i: %s - %g\n",elem->type,elem->name,node_toFloat(node_copy(elem)));
        break;        
      case BF_TYPE_STRING:
      case BF_TYPE_C_TEXT:
        printf("%i: %s - %s\n",elem->type,elem->name,elem->content);
        break;
      default:
...
int vector_replace_vector(struct vector *v,char* name,struct vector *content) {
  struct node *n = node_create(content,BF_TYPE_VECTOR);
  assert(n!=NULL);
  return vector_replace(v,name,n);
}
