    ...
    switch(z) {
      ...
      case 1:  // Zahl
        ...
        if(c=='e' || c=='E')
          z=19;  // exponent von floating-point zahl
        if(whitespace(c)) {
          addsymbol(progstring,&first,i,BF_TYPE_INT);
          z=0; // zahl fertig erkannt
        }
        break;
      case 2:  // Befehl
        if(whitespace(c)) {
          addsymbol(progstring,&first,i,BF_TYPE_C_TEXT);
          z=0;
        }
        break;
      case 3:  // Zeichenkette
        if(c=='"') {
          addsymbol(progstring,&first,i,BF_TYPE_STRING);
          z=0;
        }
        break;
      case 4:  // Vorzeichen
...
struct vector *word_load(char *name,char *ns) {
  ...
  do {
    ...
    if(ns==NULL) {
      ...
      buffer=malloc(10+strlen(lex->content)+strlen(name));
      sprintf(buffer,"command/%s/%s",lex->content,name);
      tmpns=lex->content;
    } else {
    ...
    if(n) {
      if(n->type!=BF_TYPE_INT) {
        if(n->type==BF_TYPE_VECTOR) {
          v=n->content;
          printf("wort %s im cache gefunden\n",buffer);
        }else {
          printf("falscher typ auf dem word-cache\n");
          exit(1);
        }
      } else {
  ...
  if(v) {  // sollte immer erfuellt sein
    ...
    if(n) {
      if(n->type==BF_TYPE_VECTOR) {
        printf("vorkompiliertes programm gefunden\n");
        program=n->content;
      ...
      if(program) {
        ...
        if(program->head) {
          ...
          vector_replace_vector(words,buffer,v);
        }
      }
    }
    return program;
...
void botforth_replaceword(struct vector *program) {
  ...
  cachename=(char*)((struct vector*)program->head->content)->head->next->next->next->content;
  shortname=(char*)((struct vector*)program->head->content)->head->next->content;
  ...
  if(newprogram) {
    ...
    newcachename=(char*)((struct vector*)program->head->content)->head->next->next->next->content;
...
void botforth_call(struct vector *program) {
  ...
  //if(rstack->size<50) {
    ...
    if(*(int*)((struct vector*)program->head->content)->head->next->next->content==1) {
  ...
  do {
    ...
    switch(c->type) {
      case BF_TYPE_C_TEXT:  // Befehl in Textform
        command=c->content;
        //printf("textbefehl: %s\n",command);
        if(strlen(command)>1 && command[0]=='_' && command[1]!='_') {
          temp2=infoblock_get_name(cword);
          printf("hauptfunktionsname: %s\n",temp2);
          temp=malloc(strlen(temp2)+strlen(command)+1);
      ...
      case BF_TYPE_C_EXT:   // Externer Befehl
        ...
        if(rstack->size<8000) {
          botforth_call((struct vector *)c->content);
        } else {
    ...
    while(pc==NULL && rstack->tail!=NULL || 
          (accesslevel>1 && runlimit>0 && (int)time(NULL)>runlimit)) {
      botforth_return();
    }
  } while (pc!=NULL);
  ...
  while(n!=NULL) {
    ...
    if(n->type==BF_TYPE_C_TEXT) {
      if(getintcommand(n->content)) {
        ...
        compword=word_load(n->content,"compiler");
        ...
        */
        ...
        if(compword) {
          ...
          printf("den neuen teil nochmal kompilieren\n");
...
int main(int argc, char **argv) {
  ...
  if( !mysql_real_connect(&mysql_read,"127.0.0.1","feuerzeugread","amoklauf",NULL,0,NULL,0)){
  ...
  if( !mysql_real_connect(&mysql_write,"127.0.0.1","feuerzeug","slightomat",NULL,0,NULL,0)){
  ...
  temp=malloc(4); sprintf(temp,"lib");
  vector_push_string(lexicon,temp);
  temp=malloc(4); sprintf(temp,"service");
  vector_push_string(lexicon,temp);
  temp=malloc(4); sprintf(temp,"bot");
  vector_push_string(lexicon,temp);
